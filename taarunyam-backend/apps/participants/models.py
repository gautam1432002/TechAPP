...
class Registration(models.Model):
    ...
    def _generate_registration_id(self):
        """Generate TAR-{YEAR}-{3-digit sequential number} safely."""
        from django.utils import timezone
        import random
        year = timezone.now().year
        
        # Count existing based on prefix to avoid date field extraction issues
        base_count = Registration.objects.filter(registration_id__startswith=f'TAR-{year}-').count()
        
        # Loop to find an available ID to prevent concurrent IntegrityError
        for attempt in range(20):
            seq = base_count + 1 + attempt
            candidate_id = f'TAR-{year}-{seq:03d}'
            if not Registration.objects.filter(registration_id=candidate_id).exists():
                return candidate_id
                
        # Fallback in extreme cases
        return f'TAR-{year}-{random.randint(10000, 99999)}'
